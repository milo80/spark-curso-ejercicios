package mx.ejercicios.curso.accumulator
...
/*
  Cuenta el numero de respuestVas por Pais, dentro de un flujo de mensajes
*/
object FindResponsesBitesProc {
  def main(args: Array[String]) {

    Logger.getLogger("org").setLevel(Level.ERROR)
    val conf = new SparkConf().setAppName("Responses Processes Bites").setMaster("local[1]")
    ...
    val responseFromCanada = responseRDD.filter(response => {
      ...
      println(response.getBytes().length)
